...
- **GitHub:** https://github.com/megapolisagent/MAIN_ASSISTANT
...
### INF-001 · Onboarding User Template
- **Тип:** Infrastructure
- **Путь:** `ARCHIVE/onboarding_user_template.md`
- **Задача:** Шаблон онбординга для read-only пользователя, получающего доступ к репозиториям MAIN_ENGINEER + MAIN_ASSISTANT через github Collaborator Read role
- **Pattern:** Read-only access pattern — github Read role блокирует push в origin, безопасность системы Марии не зависит от поведения локальной копии пользователя; адаптация под конкретного человека только в Шаге 5 (первая фраза агенту) + опционально Prerequisites
- **Stage:** S1 — Working Prototype (создан под кейс дочери, working loop в реальном использовании не проверен)
- **Приоритет:** MEDIUM
- **Статус:** Активен
- **Dependencies:** PRJ-001 (MAIN_ENGINEER github repo), PRJ-006 (MAIN_ASSISTANT github repo) — оба должны быть доступны для invite через Settings → Collaborators
- **Open Threads:** Поведение `MAIN_ASSISTANT_CORE.md` при «не-Мария-пользователь» — calibration signal от первой реальной сессии (дочь + travel-agent). Если CLAUDE.md/CORE требуют адаптации под внешнего юзера — это вход для следующей итерации MAIN_ASSISTANT, не для шаблона.
- **Last Verified:** 2026-06-18
- **Дата регистрации:** 2026-06-18
